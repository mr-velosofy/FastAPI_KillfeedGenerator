...
def create_killfeed(killer_name, victim_name, killer_agent, victim_agent, weapon, is_headshot=False, is_player_kill=False, is_wallbang=False, numeral=None):
    """
    ...
    KILLER_BG_COLOR = (75, 190, 158, 255) # Teal color
    VICTIM_BG_COLOR = (214, 98, 98, 255)   # Red color
    ...
    total_width = (AGENT_ICON_SIZE[0] + PADDING + killer_name_width + PADDING +
                   weapon_width + headshot_width + wallbang_width + PADDING +PADDING+
    ...
    killer_agent_hightlight = Image.new('RGBA', killer_agent_img.size, (119, 233, 199, 255))
    ...
    victim_agent_hightlight = Image.new('RGBA', victim_agent_flipped.size, (240, 146, 146, 255))
    ...
    if is_player_kill:
        # img = Image.open(os.path.join(OUTPUT_PATH, output_filename))
        img = final_image
        img2 = Image.open(os.path.join(ASSETS_PATH, 'ui', 'MeBorderTriangle.png')).convert('RGBA')

        # Change img2 color to (231, 237, 131, 255) using alpha mask
        yellow_overlay = Image.new('RGBA', img2.size, (231, 237, 131, 255))
        img2_colored = Image.new('RGBA', img2.size, (0, 0, 0, 0))
        img2_colored.paste(yellow_overlay, (0, 0), img2)  # Use alpha channel of img2 as mask
        ...
        # Paste both images
        final_image.paste(img2_colored, (0, img2_y), img2_colored)
        final_image.paste(img, (img2.width, 0), img)

        # Save with new filename
        final_output_filename = output_filename
        final_image.save(os.path.join(OUTPUT_PATH, final_output_filename), optimize=False)
        print(f"✅ Applied Me Highlight to {os.path.join(OUTPUT_PATH, final_output_filename)}")
    ...
    if numeral:
        ...
        if is_player_kill:
            ...
            final_image.paste(numeral_img, (76, 30), numeral_img)
        else:
            final_image = Image.new('RGBA', (new_width, new_height), (0, 0, 0, 0))
            final_image.paste(image, (numeral_img.width, 0), image)
    ...
    victim_bg_shape_shifted = [(x + offset_x, y) for x, y in victim_bg_shape]

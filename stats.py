...
ONLINE_WINDOW = 180.0        # seconds a visitor counts as "online"
PREVIEW_THROTTLE = 10.0      # min seconds between logged previews per visitor

enabled = bool(DATABASE_URL)

# ── in-memory state ──────────────────────────────────────────────────────────
_presence = {}                      # vid -> monotonic last-seen
_presence_lock = threading.Lock()
_peak_online = 0
_last_preview = {}                  # vid -> monotonic (throttle)
_stats_task = None
_last_digest_at = None              # datetime UTC of last posted digest
_public_cache = {"at": 0.0, "data": {"online": 0, "today": 0}}


def ensure_vid(raw):
    """Return (vid, is_new_cookie) from an incoming vf_vid cookie value."""
...
def heartbeat(vid):
    """Register presence; returns current online count."""
    now = time.monotonic()
    with _presence_lock:
        _presence[vid] = now
        _sweep(now)
...
def _run_bg(fn, *args):
    """Fire-and-forget work in a daemon thread so requests never wait on DB."""
    ...
    try:
        with _connect() as c:
            row = c.execute(
                ...
                "ON CONFLICT (vid) DO UPDATE SET last_seen = now() RETURNING first_seen",
                (vid,)).fetchone()
            returning = row["first_seen"].date() < datetime.now(timezone.utc).date()
            c.execute(
                "INSERT INTO daily_visits(day, vid, returning) VALUES (CURRENT_DATE, %s, %s) "
                "ON CONFLICT DO NOTHING",
                (vid, returning))
            if ref_host:
                ref_host = ref_host[:100].lower()
                c.execute(
                    "INSERT INTO referrers(day, host, count) VALUES (CURRENT_DATE, %s, 1) "
...
def page_hit(vid, referer=None):
    """Called once per page view: presence + visit + referrer."""
    heartbeat(vid)
    if enabled:
        host = None
        if referer:
...
def log_event(kind, vid="", **fields):
    """kind: 'preview' | 'export'. Previews throttled per visitor."""
    if not enabled:
        return
    now = time.monotonic()
    if kind == "preview":
...
def public_stats():
    ...
    try:
        with _connect() as c:
            row = c.execute(
                "SELECT count(*) AS n FROM events WHERE ts >= date_trunc('day', now())"
            ).fetchone()
...
def _scalar(c, sql, *params):
    return int(c.execute(sql, params or None).fetchone()["n"])


def collect_digest(since):
    """Aggregate every stat block for the embed. Returns a plain dict."""
    with _connect() as c:
        gens_since = _scalar(c, "SELECT count(*) AS n FROM events WHERE ts > %s", since)
        split = c.execute("""
            SELECT kind, count(*) AS n FROM events
            WHERE ts > %s GROUP BY kind""", (since,)).fetchall()
        by_kind = {r["kind"]: int(r["n"]) for r in split}

        def window_count(table, where):
            return _scalar(c, f"SELECT count(*) AS n FROM {table} WHERE {where}")

        gens = {
            "day": window_count("events", "ts >= date_trunc('day', now())"),
            "week": window_count("events", "ts >= now() - interval '7 days'"),
            "month": window_count("events", "ts >= date_trunc('month', now())"),
            "all": window_count("events", "TRUE"),
        }
        uniq = {
        ...
        nr = c.execute(
            "SELECT returning, count(*) AS n FROM daily_visits WHERE day = CURRENT_DATE "
            "GROUP BY returning").fetchall()
        new_ret = {str(r["returning"]).lower(): int(r["n"]) for r in nr}

        visits = {
            "day": window_count("daily_visits", "day = CURRENT_DATE"),
            "week": window_count("daily_visits", "day >= CURRENT_DATE - 6"),
            "month": window_count("daily_visits", "day >= date_trunc('month', now())"),
            "all": window_count("daily_visits", "TRUE"),
        }
    ...
    return {
        ...
        "mode_mix": [(m, round(100 * n / mode_total)) for m, n in
                     sorted(modes_map.items(), key=lambda kv: -kv[1])],
...
def build_embed(d):
    """Turn collected stats into a Discord embed payload dict."""
    def chain(vals):
    ...
    uniq_today = d["uniq"]["day"]
    uniq_line = (
        f"Today: **{uniq_today}** ({d['today_new']} new / {d['today_returning']} ret)\n"
    ...
    if d["top_weapons"]:
        fields.append({"name": "⚔️ Top weapons (7d exports)", "value": chain([f"{w} ({n})" for w, n in d["top_weapons"]]), "inline": True})
    if d["mode_mix"]:
...
def post_digest():
    ...
    payload = build_embed(collect_digest(since))
    req = urllib.request.Request(
        WEBHOOK_URL,
        data=b"payload_json=" + urllib.parse.quote(json.dumps(payload)).encode(),

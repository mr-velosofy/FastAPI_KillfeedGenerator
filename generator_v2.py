...
def create_killfeed(killer_name, victim_name, killer_agent, victim_agent, weapon,
    ...
    tw = int(right_text_x + vw + TEXT_PADDING + AGENT_ICON_SIZE[0])
    ...
    canvas.paste(Image.new("RGBA", (120, 130), (*RED_HIGHLIGHT, 255)), (tw - 120, 0))
    ...
    # Agent icons
    kill_hl_color = YELLOW if is_player_kill else TEAL_HIGHLIGHT
    ...
    victim_hl = Image.new("RGBA", victim_flipped.size, (*RED_HIGHLIGHT, 255))
    ...
    # Me border on top
    if is_player_kill:
        mask = create_custom_me_mask(mx)
        me_hl = Image.new("RGBA", mask.size, (*YELLOW, 255))
        canvas.paste(me_hl, (-30, 0), mask)
